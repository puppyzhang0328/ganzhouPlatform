...
function addHistryCar(req) {
    ...
    if(req.location == 2){
        if(req.plate_number){
            var base = new Base64();
          var  plate_number = base.decode(req.plate_number).toString();
        }else {
            var plate_number=""
        }
        // var plate_number=encodeURI(encodeURI(req.plate_number))
        //解析成明码
        // var base = new Base64();
        // plate_number = base.decode(req.plate_number).toString();
    }else {
        var plate_number;
        plate_number = decodeURI(req.plate_number);
    ...
    if(req.plate_number == 'bnVsbA'){
        $.get(urls+'/parkhero/v0.1/billing/onlinepay/',{'inids':req.inid,'paytype':'wechatpay','servicetype':'parkingpay','openid':req.openid,'identifier':req.identifier,'location':req.location,toller_tel:req.phone_number,timestamp:getTimestamp},function(response){
            window.response = '';
        ...
        $.get(urls+'/parkhero/v0.1/billing/onlinepay/',{'plate_number':plate_number,'paytype':'wechatpay','servicetype':'parkingpay','openid':req.openid,'identifier':req.parkingid,'location':req.location,toller_tel:req.phone_number,timestamp:getTimestamp,'out_trade_no':req.out_trade_no,'gate':req.gate},function(response){
            window.response = '';
            if(response.status == '0'){
                closeMask();
                window.response = '';
                ...
                $('#icoTips').css('font-size','12px')
                $('#icoTips').html('尚未产生费用，请直接离场')
                $('#amount').html("0.00");
                $('.tips').css('display','block');
                $('#charged_duration').html(charged_duration);
                $('#parking_time').html(response.trade_no.parking_time);
                $('#parking_lot').text(response.trade_no.parking_lot);
                $('#list_free').css('display','block');
                $('#parking_free').html(response.trade_no.no_charge_time+'分钟')
            }else if(response.status == '30008'){
                closeMask();
                alert('订单已支付');
            }else if(response.status == '30011'){
        ...
        wx.ready(function () {
            ...
            wx.chooseWXPay({
                ...
                success: function (res) {
                    ...
                    sessionStorage.setItem("free_outtime", response.trade_no.menuinfo.free_outtime);
                    window.open(urls+'/3rdpay/H5_pay/pay_success.html','_self')
                    // sessionStorage.setItem("lot_type", response.trade_no.menuinfo.lot_type);
                    // window.open('http://dev.dadapark.com/3rdpay/H5_pay/pay_success.html','_self')
                },
                cancel: function (res) {
                    closeMask();
                    alert('你已取消付款');
