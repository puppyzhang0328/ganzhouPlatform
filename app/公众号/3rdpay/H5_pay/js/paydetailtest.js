...
(function ($) {
    ...
    $('#pay_btn').on('click',function () {
        sessionStorage.setItem("free_outtime", '15');
        window.open(urls+'/3rdpay/H5_pay/pay_successtest.html','_self')
        openMask();
        onBridgeReady(req);
    });
})(Zepto);
...
function addHistryCar(req) {
    ...
    if(req.plate_number == 'bnVsbA'){
        $.get(urls+'/parkhero/v0.1/billing/onlinepay/',{'inids':req.inid,'paytype':'wechatpay','servicetype':'parkingpay','openid':req.openid,'identifier':req.identifier,'location':req.location,toller_tel:req.phone_number},function(response){
            window.response = '';
        ...
        $.get(urls+'/parkhero/v0.1/billing/onlinepay/',{'plate_number':plate_number,'paytype':'wechatpay','servicetype':'parkingpay','openid':req.openid,'identifier':req.parkingid,'location':req.location,toller_tel:req.phone_number},function(response){
            window.response = '';
            if(response.status == '0'){
                closeMask();
                window.response = '';
