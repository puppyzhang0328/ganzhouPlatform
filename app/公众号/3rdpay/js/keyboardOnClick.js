...
$("#checkbox").change(function() {
    if($("input[type='checkbox']").is(':checked')){
        ...
        $("#7").show();
        $('#nowBox').css('border','none');
        $('.box').css('background','rgb(79,182,64)');
        $('.box').css('border','1.5px solid');
        $(".input").addClass("colorFff");
        $(".input").addClass("borderRight");
        $("#7").text('');
    }else {
        $("#7").text('');
        $("#7").hide();
        $(".input").removeClass("input_change");
        $('#nowBox').addClass('nowBox');
        $('.box').css('border','none');
        $('#nowBox').css('border','2px solid #fff');
        $('.box').css('background','rgb(53,120,199)');
        $(".input").removeClass("colorFff");
        $(".input").removeClass("borderRight");
    }
});
//完成按钮
