...
/***/ function(module, exports) {
	...
	var redrawMarkers = exports.redrawMarkers = function redrawMarkers(map, previousMarkers, opts) {
	    ...
	    opts.markers.forEach(function (marker) {
            ...
            // var label = new BMap.Label(marker.label, { offset: new BMap.Size(30, -10) });
	        ...
	        // var msg = '<h3>' + (marker.title || '') + '</h3><div class="progress progress-striped" style="margin-bottom:0px;background: #f1f1f1;color:#fff;"><div class="progress-bar '+marker.color+'" style="width: '+marker.proportion+'%"><div class="ng-binding" style="white-space: nowrap;color: #333;">'+(marker.content || '')+'</div></div></div><div>'+'<img src="'+marker.img+'" alt="暂无照片" height="200px" style="width:100%"></div>';
            var msg = '<h3>' + (marker.title || '') + '</h3><div>'+'<img src="'+marker.img+'" alt="暂无照片" height="200px" style="width:100%"></div>';
