(function () {
    ...
    parkStateController.$inject = ['ManageParkingService', 'ParkingStateService', 'DTColumnDefBuilder', 'datatablesOptions','$document','logger','URL_SEED','$scope','$timeout'];
    function parkStateController(ManageParkingService, ParkingStateService, DTColumnDefBuilder, datatablesOptions,$document,logger,URL_SEED,$scope,$timeout) {
        var vm = this;
...
//不在线的上传工具
        ...
        function readRecords(start_index){
            if(start_index == 0){//首页index
                ParkingStateService.readRecords(vm.pagination.querytype.uptool,0,"").then(function (response) {
                    vm.tool_offline_num = [];
                    ...
                    vm.start_number=vm.tool_offline_num[vm.tool_offline_num.length-1].number;
                ...
                if (vm.pagination.previous_start_index >= vm.pagination.maxid) {
                    ...
                    ParkingStateService.readRecords(vm.pagination.querytype.uptool,vm.pagination.previous_start_index,0).then(function (response) {
                        vm.tool_offline_num = [];
                        ...
                        //排序的索引
                        vm.next_number=vm.tool_offline_num[0].number;
                        vm.start_number=vm.tool_offline_num[vm.tool_offline_num.length-1].number;
                        //测试服务器排序
                ...
                if (vm.pagination.next_start_index <= vm.pagination.minid) {
                    ...
                    ParkingStateService.readRecords(vm.pagination.querytype.uptool,vm.pagination.next_start_index,1).then(function (response) {
                        vm.tool_offline_num = [];
                        angular.forEach(response.records, function (value,index) {
                ...
                ParkingStateService.readRecords(vm.pagination.querytype.uptool,-1,"").then(function (response) {
                    vm.tool_offline_num = [];
                    angular.forEach(response.records, function (value,index) {
                        value.number=2163+(index+1);
                        value.htttpName = '已接入';
                        vm.tool_offline_num.push(value);
        ...
        function onlineupload() {
            ParkingStateService.onlineRecords(vm.pagination.querytype.uptool,vm.pagination.state.online).then(function (response) {
