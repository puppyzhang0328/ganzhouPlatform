(function () {
    angular.module('app.prepayment')
        .controller('OnlinePayBcspayRechargeController',OnlinePayBcspayRechargeController);
    OnlinePayBcspayRechargeController.$inject = ['OnlinePaymentService', '$scope', 'datatablesOptions','DTColumnDefBuilder','$document','billOnlineFactory','$location'];
    'use strict';
    function OnlinePayBcspayRechargeController(OnlinePaymentService, $scope, datatablesOptions,DTColumnDefBuilder,$document,billOnlineFactory,$location) {
        var vm = this;
                ...
                OnlinePaymentService.readRecords($scope.pagination.previous_start_index, $scope.pagination.pageSize, $scope.pagination.pagetype.pervious_page,$scope.servicetype,$scope.payment_channel,$scope.username,$scope.paid,vm.valid_begintime,vm.valid_endtime,vm.plate_number).then(function (response) {
                    ...
                    angular.forEach(response.records, function (value,index) {
                        ...
                        switch (value.payment_channel){
                            ...
                            case 'bcspay': $scope.recordsData[index].payment_channelName ='长沙银行';break;
                        }
                    });
