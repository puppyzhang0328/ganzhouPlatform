...
(function () {
    ...
    angular.module('app.unnormalbill')
        .controller('unNormalbillController',unNormalbillController);
    unNormalbillController.$inject = ['unNormalBillServe', '$scope', 'datatablesOptions','DTColumnDefBuilder','$document','$location','$uibModal','normalBillFactory'];
    function unNormalbillController(unNormalBillServe, $scope, datatablesOptions,DTColumnDefBuilder,$document,$location,$uibModal,normalBillFactory) {
        var vm = this;
        ...
        $scope.loadPreviousPage = function () {
            if ($scope.pagination.previous_start_index >= $scope.pagination.maxid) {
                ...
                unNormalBillServe.readRecords($scope.pagination.previous_start_index,0,'xxtx',vm.valid_begintime,vm.valid_endtime).then(function (response) {
                    ...
                    if(vm.next_number-100<=0){
                        angular.forEach(response.records, function (value,index) {
                            ...
                            if(value.thirdpay_checkstatus=='102'){
                                $scope.recordsData[index].paidName = '第三方支付不在存在';
                            }else if(value.thirdpay_checkstatus=='103') {
                                $scope.recordsData[index].paidName = '第三方支付金额不一致';
                            }else if(value.thirdpay_checkstatus=='104'){
                                $scope.recordsData[index].paidName = '支付时间不一致 ';
                            }else if(value.thirdpay_checkstatus=='0'){
                                $scope.recordsData[index].paidName = '对账一致';
                            }else if(value.thirdpay_checkstatus=='106'){
                                $scope.recordsData[index].paidName = '异常已处理';
                            }else {
                                $scope.recordsData[index].paidName = '尚未对账';
                            }
                            ...
                            if(value.old_checkstatus=='102'){
                                $scope.recordsData[index].oldName = '第三方支付不在存在';
                            }else if(value.old_checkstatus=='103') {
                                $scope.recordsData[index].oldName = '第三方支付金额不一致';
                            }else if(value.old_checkstatus=='104'){
                                $scope.recordsData[index].oldName = '支付时间不一致 ';
                            }else if(value.old_checkstatus=='0'){
                                $scope.recordsData[index].oldName = '对账一致';
                            }else if(value.old_checkstatus=='106'){
                                $scope.recordsData[index].oldName = '异常已处理';
                            }else {
                                $scope.recordsData[index].oldName = '尚未对账';
                            }
        ...
        $scope.loadNextPage = function () {
            if ($scope.pagination.next_start_index <= $scope.pagination.minid) {
                ...
                unNormalBillServe.readRecords($scope.pagination.next_start_index,1,'xxtx',vm.valid_begintime,vm.valid_endtime).then(function (response) {
                    $scope.recordsData = response.records;
                    angular.forEach(response.records, function (value,index) {
        ...
        $scope.loadFistPage = function () {
            ...
            unNormalBillServe.readRecords(0,"",'xxtx',vm.valid_begintime,vm.valid_endtime).then(function (response) {
        ...
        $scope.loadLastPage = function () {
            ...
            unNormalBillServe.readRecords(-1,"",'xxtx',vm.valid_begintime,vm.valid_endtime).then(function (response) {
                $scope.recordsData = response.records;
                angular.forEach(response.records, function (value,index) {
