...
(function () {
    ...
    angular.module('app.unnormalbill')
        .controller('appBillController',appBillController);
    appBillController.$inject = ['unNormalBillServe', '$scope', 'datatablesOptions','DTColumnDefBuilder','$document','$location','normalBillFactory','$uibModal'];

    function appBillController(unNormalBillServe, $scope, datatablesOptions,DTColumnDefBuilder,$document,$location,normalBillFactory,$uibModal) {
        var vm = this;
        moment.locale('zh-cn', {
            longDateFormat: {
                LT: 'HH:mm',
        ...
        $scope.loadPreviousPage = function () {
            if ($scope.pagination.previous_start_index >= $scope.pagination.maxid) {
                ...
                unNormalBillServe.readRecords($scope.pagination.previous_start_index,0,'app',vm.valid_begintime,vm.valid_endtime).then(function (response) {
                    ...
                    if(vm.next_number-100<=0){
                        angular.forEach(response.records, function (value,index) {
                            ...
                            if(value.thirdpay_checkstatus =='103'){
                                $scope.recordsData[index].paidName = '与上次对账金额不一致';
                            }else if(value.thirdpay_checkstatus=='102') {
                                $scope.recordsData[index].paidName = '第三方不存在';
                            ...
                            switch (value.payment_channel){
                                case 'dadapay': $scope.recordsData[index].payment_channelName='App支付';break;
        ...
        $scope.loadNextPage = function () {
            if ($scope.pagination.next_start_index <= $scope.pagination.minid) {
                ...
                unNormalBillServe.readRecords($scope.pagination.next_start_index,1,'app',vm.valid_begintime,vm.valid_endtime).then(function (response) {
        ...
        $scope.loadFistPage = function () {
            ...
            unNormalBillServe.readRecords(0,"",'app',vm.valid_begintime,vm.valid_endtime).then(function (response) {
...
{}                        switch (value.payment_channel){
        ...
        $scope.loadLastPage = function () {
            ...
            unNormalBillServe.readRecords(-1,"",'app',vm.valid_begintime,vm.valid_endtime).then(function (response) {
                $scope.recordsData = response.records;
                angular.forEach(response.records, function (value,index) {
